...
from tensorpack.callbacks import (DataParallelInferenceRunner, ModelSaver,
                                  MaxSaver, ScheduledHyperParamSetter)
...
class StatCollector(Inferencer, Config):
    ...
    def _after_inference(self):
        ...
        if self.model_type == 'dist':
        ...
        elif (self.model_type == 'np_dg' or self.model_type == 'mynetwork') and self.regression:
            pred_dg = pred_inst[..., -1]
            ...
            for i in range(true_ng.shape[0]):
                ssim_ng.append(ssim_metric(true_ng[i], pred_ng[i], data_range=1.0, win_size=11, K1=0.01, K2=0.03,
                                           gaussian_weights=1.5))
                ssim_pg.append(ssim_metric(true_pg[i], pred_pg[i], data_range=1.0, win_size=11, K1=0.01, K2=0.03,
                                           gaussian_weights=1.5))
